...
document.querySelector('.js-move').addEventListener('click',()=>{
  write('SINGLE_MOVEMENT.Command.MoveDirectAbsolute.ExecuteCmd',1);
  write('SINGLE_MOVEMENT.Command.MoveDirectAbsolute.ExecuteCmd',0);
});

document.querySelector('.js-read-Tpoint').addEventListener('click',()=>{
  write('SINGLE_MOVEMENT.Command.PTP_ReadPositionFrom',1);
  readPosition();
});
...
function readPosition(){
  //READ AXIS POSITIONS
  $(document).ready(function(){
      $.get(`5-var-10.htm`, function(result){
        word = ((result.trim()));
        let variab='';
        let neword='';
        arr5=[]; 
