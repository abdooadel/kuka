...
for(let i=1;i<7;i++){
  document.querySelector(`.js-input-a${i}`).addEventListener('keydown',(event)=>{
    if(event.key==='Enter'){
    write(`SINGLE_MOVEMENT.Command.MoveAxisAbsolute.AxisPosition.A[${i}].Point_Name`,document.querySelector(`.js-input-a${i}`).value);
    document.querySelector(`.js-input-a${i}`).placeholder=document.querySelector(`.js-input-a${i}`).value;
    }
  });
...
document.querySelector('.js-move').addEventListener('click',()=>{
  write('SINGLE_MOVEMENT.Command.MoveAxisAbsolute.ExecuteCmd',1);
  write('SINGLE_MOVEMENT.Command.MoveAxisAbsolute.ExecuteCmd',0);
});

for(let selectX=1; selectX<7; selectX++){
  document.querySelector(`.js-input-a${selectX}`).addEventListener('click',()=>{
    document.querySelector(`.js-input-a${selectX}`).placeholder=''
  })
}
  ...
  function readMovementStatus(){
    ...
    function readPosition(){
      $(document).ready(function(){
        $.ajaxSetup({ cache: false });
       // setInterval(function() {
          $.get(`5-var-6.htm`, function(result){
            let word1 = ((result.trim()));
              ...
              {
                arr5[i]=0.00;
                 document.querySelector(`.js-input-a${i+1}`).placeholder = `${(Number(arr5[i])).toFixed(2)}  ${String.fromCharCode(176)} `;
              }
              else if (strg[0]==='&') {
                strg= strg.slice(6,strg.length);
                arr5[i]=strg*-1;
